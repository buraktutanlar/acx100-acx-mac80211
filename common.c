...
void acx_s_get_firmware_version(acx_device_t * adev)
{
	...
	acx_s_interrogate(adev, &fw, ACX1xx_REG_FWREV);
...
static int acx_s_proc_diag_output(char *buf, acx_device_t * adev)
{
	...
	if (OK != acx_s_interrogate(adev, st, ACX1xx_REG_FIRMWARE_STATISTICS))
		goto fw_stats_end;
			    ...
			    u32 * feature_options, u32 * data_flow_options)
{
	...
	if (OK != acx_s_interrogate(adev, &feat, ACX1xx_REG_FEATURE_CONFIG)) {
		FN_EXIT1(NOT_OK); 
		return NOT_OK;
	}
...
static int acx100_s_create_dma_regions(acx_device_t * adev)
{
	...
	if (OK != acx_s_interrogate(adev, &memmap, ACX1xx_REG_MEMORY_MAP)) {
		goto fail;
	}
...
static int acx100_s_init_wep(acx_device_t * adev)
{
	...
	if (OK != acx_s_interrogate(adev, &pt, ACX1xx_REG_MEMORY_MAP)) {
		goto fail;
	}
...
static int acx_s_init_packet_templates(acx_device_t * adev)
{
	...
	if (OK != acx_s_interrogate(adev, &mm, ACX1xx_REG_MEMORY_MAP))
		goto failed_acx100;
...
static void acx_s_update_80211_powersave_mode(acx_device_t * adev)
{
	...
	acx_s_interrogate(adev, &pm, ACX1xx_REG_POWER_MGMT);
...
void acx_s_update_card_settings(acx_device_t *adev)
{
	...
	if (adev->get_mask & GETSET_STATION_ID) {
		...
		acx_s_interrogate(adev, &stationID, ACX1xx_REG_DOT11_STATION_ID);
		paddr = &stationID[4];
