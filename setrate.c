...
static const u8 ratelist[] =
    { 1, 2, 5, 11, 22, 33, 6, 9, 12, 18, 24, 36, 48, 54 };
static const u8 dot11ratebyte[] =
    { 1 * 2, 2 * 2, 11, 11 * 2, 22 * 2, 33 * 2, 6 * 2, 9 * 2, 12 * 2, 18 * 2,
24 * 2, 36 * 2, 48 * 2, 54 * 2 };
...
static int get_modulation(int r_enum, char suffix)
{
	...
	if (suffix == 'c') {
		if (r_enum < DOT11_RATE_5 || r_enum > DOT11_RATE_11)
			return -EINVAL;
		return DOT11_MOD_CCK;
	}
	if (suffix == 'p') {
		if (r_enum < DOT11_RATE_5 || r_enum > DOT11_RATE_33)
			return -EINVAL;
		return DOT11_MOD_PBCC;
	}
	if (suffix == 'o') {
		if (r_enum < DOT11_RATE_6)
			return -EINVAL;
...
fill_ratemask(const char **pstr, u32 * mask,
	      int (*supported) (int mbit, int mod, void *opaque),
	      u32(*gen_mask) (int mbit, int mod, void *opaque), void *opaque)
{
	unsigned long rate_mbit;
	int rate_enum, mod;
	u32 m = 0;
