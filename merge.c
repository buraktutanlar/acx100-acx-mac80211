...
static void acx_create_rx_desc_queue(acx_device_t * adev, u32 rx_queue_start)
{
	...
	if (IS_ACX111(adev)) {
		...
		if (IS_PCI(adev))
			...
			mem_offs = (uintptr_t) adev->rx.desc_start;
			while (mem_offs < (uintptr_t) adev->rx.desc_start
				+ (RX_CNT * sizeof(*rxdesc))) {
			...
			unsigned cmd_timeout, const char *cmdstr)
{
	...
	if (buffer && buflen) {
		...
		if (IS_MEM(adev)) {
			...
			/* adev->cmd_area points to PCI device's
			 * memory, not to RAM! */
	...
	if (cmd_status != 1) { /* it is not a 'Success' */
		...
		/* zero out result buffer WARNING: this will trash
		 * stack in case of illegally large input length! */
