...
int acx100_process_disassociate(wlan_fr_disassoc_t *req, wlandevice_t *priv)
{
	...
	if (WLAN_GET_FC_TODS(ieee2host16(hdr->a3.fc)) || WLAN_GET_FC_FROMDS(ieee2host16(hdr->a3.fc)))
		...
		if (ACX_MODE_0_IBSS_ADHOC == priv->macmode_joined)
			res = 1;
		else if (OK == acx100_is_mac_address_equal(priv->dev_addr, hdr->a3.a1 /* RA */)) {
			res = 1;
...
static int acx100_process_data_frame_master(struct rxhostdescriptor *rxdesc, wlandevice_t *priv)
{
	...
	if (memcmp(bssid, priv->bssid, ETH_ALEN) == 0) {
		if (!(clt = acx100_sta_list_get(priv, bcast_addr)) || (clt->used != 3)) {
			...
			if (OK != acx100_is_mac_address_broadcast(da)) {
...
static int acx100_process_data_frame_client(struct rxhostdescriptor *rxdesc, wlandevice_t *priv)
{
	...
	/* check if it is our bssid */
        if (OK =! acx100_is_mac_address_equal(priv->bssid, bssid)) {
	...
	if (OK != acx100_is_mac_address_broadcast(da)) {
		if ((signed char) da[0] >= 0) {
			/* no broadcast, so check if it is our address */
                        if (OK != acx100_is_mac_address_equal(da, priv->dev_addr)) {
			  ...
			  acxp80211_hdr_t *hdr)
{
	...
	for (station = 0; station < priv->bss_table_count; station++) {
		...
		if (OK == acx100_is_mac_address_equal
		    (hdr->a4.a3,
...
/*	acx_ie_generic_t pdr; */
	...
	if (WLAN_GET_FC_TODS(ieee2host16(hdr->a4.fc)) || WLAN_GET_FC_FROMDS(ieee2host16(hdr->a4.fc)))
		...
		if (OK == acx100_is_mac_address_equal(priv->dev_addr, hdr->a4.a1 /* RA */)) {
			if (WLAN_MGMT_STATUS_SUCCESS == req->status[0]) {
				priv->aid = req->aid[0]; /* FIXME: huh?? this didn't get set, or did it? */
				...
				SET_BIT(priv->after_interrupt_jobs, ACX100_AFTER_INTERRUPT_CMD_ASSOCIATE);
		      ...
		      wlandevice_t *priv)
{
	...
	if (sta_list != NULL) {
		...
		payload->auth_alg = host2ieee16(*(arg_0->auth_alg));
...
int acx100_transmit_authen3(wlan_fr_authen_t *arg_0, wlandevice_t *priv)
{
	...
	*/
	...
	payload->auth_seq = cpu_to_le16(3);
...
int acx100_transmit_authen4(wlan_fr_authen_t *arg_0, wlandevice_t *priv)
{
	...
	payload->auth_alg = *(arg_0->auth_alg);
	payload->auth_seq = cpu_to_le16(4);
	payload->status = 0;
...
void acx100_complete_dot11_scan(wlandevice_t *priv)
{
	...
	for (idx = 0; idx < priv->bss_table_count; idx++) {
		...
		if (OK != acx100_is_mac_address_broadcast(priv->ap))
			if (OK != acx100_is_mac_address_equal(this_bss->bssid, priv->ap))
				continue;
...
void ActivatePowerSaveMode(wlandevice_t *priv, /*@unused@*/ int vala)
{
       ...
       acx100_interrogate(priv, &pm, ACX1xx_IE_POWER_MGMT);
       ...
       acx100_configure(priv, &pm, ACX1xx_IE_POWER_MGMT);
       FN_EXIT(0, 0);
}
#endif
...
#if 0
			acx100_interrogate(priv,&status,ACX1xx_IE_SCAN_STATUS);
